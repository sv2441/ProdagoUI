...
def result(df):
    ...
    Action_schema = ResponseSchema(name="Action",
                                description="List of actionables in which each point has an action word, the subject and the activity.")
    ...
    title_template = """ \ You are an AI Governance bot. Just Execute the set of steps.
                    1)provide List broad actionables relating to "{topic}"as pre-requisites and post-requisites.Ensure that the actionables are activites and 
                    Ensure each point has an action word, the subject and the activity.
                    {format_instructions}
                    """
